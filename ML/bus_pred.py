...
def set_data():
    ...
    data = {
        'Date': pd.date_range(start='2023-03-17', periods=80*17, freq='H'),
        'Hour': list(range(6, 23)) * 80, 
        'Passengers': flatten_2d_list(datas), 
    }
...
def w_model_eval():
    ...
    time_steps = 17*5  # 하루에 해당하는 시간대 수
    next_day_steps = 17*5  # 다음날 하루에 해당하는 시간대 수
    ...
    model.save("w_lstm_model")

    return X_test
    
def w_model_pred(data):
    loaded_model = load_model("w_lstm_model")
    y_pred = loaded_model.predict(data)
    return y_pred[0]
